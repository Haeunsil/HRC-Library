...
if ($action eq "get_data") {
    ...
    eval {
        ...
        $query = "SELECT QuestionQnum, CAST(QuestionSourceQM AS NVARCHAR(MAX)) AS QuestionSourceQM, CAST(QuestionSourcePERL AS NVARCHAR(MAX)) AS QuestionSourcePERL, QuestionTag FROM [WEBDEMO].[dbo].[LibraryList] WHERE QuestionQnum IS NOT NULL ORDER BY QuestionQnum";
        ...
        while (my $row = $sth->fetchrow_hashref()) {
            ...
            my $perlcode = $row->{QuestionSourcePERL};
            ...
            if (!defined($perlcode) || $perlcode eq "" || $perlcode eq "NULL") {
                $perlcode = "";  # 공백으로 설정
            } else {
                # 문자열 정리 (앞뒤 공백 제거)
                $perlcode =~ s/^\s+|\s+$//g;
    ...
    eval {
        ...
        $query = "SELECT QuestionQnum, QuestionSourceQM, QuestionSourcePERL FROM [WEBDEMO].[dbo].[LibraryList] WHERE QuestionQnum = 1";
        $sth = $dbh->prepare($query);
    ...
    eval {
        ...
        $query = "SELECT QuestionQnum, QuestionSourceQM, QuestionSourcePERL FROM [WEBDEMO].[dbo].[LibraryList] WHERE QuestionQnum IS NOT NULL ORDER BY QuestionQnum";
        $sth = $dbh->prepare($query);
